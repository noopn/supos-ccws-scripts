...
const APP_FLOOR_API = (_, path, id) =>
  `/api/app/resource/folder?path=${path}&appId=${id}`;

const APP_FILES_API = (_, path, id) =>
  `/api/app/resource/file?path=${path}&appId=${id}&current=1&pageSize=100`;

const APP_CREATE_FOLDER_API = "/api/app/resource/folder";

const APP_CREATE_FILE_API = "/api/app/manager/uploadResource";

const CURRENT_USER_API = "/inter-api/auth/v1/currentuser";

const VERSION_API = "/api/config/version";
